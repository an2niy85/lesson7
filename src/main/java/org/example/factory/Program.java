...
public class Program {
    ...
    static  Employee generateEmployee(EmployeeType employeeType){
        String[] names = new String[] { "Анатолий", "Глеб", "Клим", "Мартин", "Лютик"};
        String[] surnames = new String[] { "Григорьев", "Фокин", "Шестаков", "Хабенский"};

        int salary = random.nextInt(200, 500);
        int index = random.nextInt(100, 150);

        return switch (employeeType){
            case Worker -> new Worker(names[random.nextInt(4)],
                    surnames[random.nextInt(4)],
                    salary * index);
            case Freelancer -> new Worker(names[random.nextInt(4)],
                    surnames[random.nextInt(4)],
                    salary);
        };
    }
